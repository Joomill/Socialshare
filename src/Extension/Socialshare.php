...
class Socialshare extends CMSPlugin implements SubscriberInterface
{
	...
	public function onContentAfterTitle(AfterTitleEvent $event): void
	{
		...
		// In category/featured view the event item is a partial object; load the full record for
		// routing and metadata. $article (the event item) is kept for output injection.
		...
		if ($view == 'featured' || $view == 'category')
		{
			...
			if ($data !== null && !isset($article->catid))
			{
				$sourceArticle = $data;
			}
		}
		...
		if (!isset($sourceArticle->catid))
		{
			return;
		}

		$currentCategory = (int) ($sourceArticle->catid ?? 0);
		...
		$slug    = $sourceArticle->alias ? ($sourceArticle->id . ':' . $sourceArticle->alias) : $sourceArticle->id;
		$siteURL = Uri::getInstance()->toString(['scheme', 'host', 'port']);
		$itemURL = $siteURL . Route::_(RouteHelper::getArticleRoute($slug, $sourceArticle->catid));
		...
		$socialPlatforms = [
			...
			'twitter' => [
				...
				'url'         => "https://twitter.com/intent/tweet?text=" . urlencode($sourceArticle->title . ': ' . $itemURL),
			...
			'linkedin' => [
				...
				'url'         => "http://www.linkedin.com/shareArticle?url=" . urlencode($itemURL) . "&title=" . urlencode($sourceArticle->title) . "&summary=" . urlencode(HTMLHelper::_('string.truncate', $sourceArticle->text, 50, false, false)),
			...
			'threads' => [
				...
				'url'         => "https://www.threads.net/intent/post?text=" . urlencode($sourceArticle->title . ' ' . $itemURL),
			...
			'mastodon' => [
				...
				'url'         => 'https://share.joinmastodon.org/#text=' . urlencode($sourceArticle->title . ' ' . $itemURL),
			...
			'bluesky' => [
				...
				'url'         => "https://bsky.app/intent/compose?text=" . urlencode($sourceArticle->title) . " %0A" . urlencode($itemURL),
			...
			'pinterest' => [
				...
				'url'         => "http://pinterest.com/pin/create/button/?url=" . urlencode($itemURL) . "&description=" . urlencode($sourceArticle->title),
			...
			'reddit' => [
				...
				'url'         => "https://www.reddit.com/submit?url=" . urlencode($itemURL) . "&title=" . urlencode($sourceArticle->title),
			...
			'tumblr' => [
				...
				'url'         => "https://www.tumblr.com/widgets/share/tool?canonicalUrl=" . urlencode($itemURL) . "&title=" . urlencode($sourceArticle->title),
			...
			'whatsapp' => [
				...
				'url'         => $isMobile ? "whatsapp://send?text=" . urlencode($sourceArticle->title . ' ' . $itemURL) : "https://web.whatsapp.com/send?text=" . urlencode($sourceArticle->title . ' ' . $itemURL),
			...
			'telegram' => [
				...
				'url'         => "https://telegram.me/share/url?url=" . urlencode($itemURL) . "&text=" . urlencode($sourceArticle->title),
			...
			'flipboard' => [
				...
				'url'         => "https://share.flipboard.com/bookmarklet/popout?v=2&url=" . urlencode($itemURL) . "&title=" . urlencode($sourceArticle->title),
			...
			'trello' => [
				...
				'url'         => "https://trello.com/add-card?url=" . urlencode($itemURL) . "&name=" . urlencode($sourceArticle->title),
			...
			'email' => [
				...
				'url'         => "mailto:?subject=" . urlencode($sourceArticle->title) . "&body=" . urlencode($itemURL),
